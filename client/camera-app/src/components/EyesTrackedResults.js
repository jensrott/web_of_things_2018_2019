...
class EyesTrackedResults extends Component {
  ...
  getThemAll = (orderBy, order) => { // Function to get all items from the database instead of one item
    firestore.collection('eyes_tracked').orderBy(orderBy, order).get().then((snapshot) => {
  ...
  handleDeleteItem = () => {
    ...
    for (let i = 0; i < selected.length; i++) {
      ...
      firestore.collection('eyes_tracked').doc(documentId).delete().then(() => {
...
EyesTrackedResults.propTypes = {
  classes: PropTypes.object.isRequired,
};

/* We can use this way theme with material ui */
export default withStyles(styles.default.mainStyles)(EyesTrackedResults);
